...
vec3 grad(float hash) {
    ...
    vec3 cuboct = cube;
    // PATCH EQSMP: índice dinâmico em vec3 crasha o compilador GLSL da Intel
    // (driver 8250-8330) — TF issue #2496 / IGCIT #1273. Versão branchless equivalente:
    int cuboctIdx = int(hash / 16.0);
    cuboct *= vec3(float(cuboctIdx != 0), float(cuboctIdx != 1), float(cuboctIdx != 2));
